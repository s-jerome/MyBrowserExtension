const Netflix = (function () {
	...
	return {
		 ...
		 * @param {any} videoData 
		 */
		async saveVideoDataAsync(operation, reason, videoData) {
			let result = {
				error: null,
				result: null
			};
			...
			try {
				...
				console.error(new Date().toLocaleString() + " -- [Netflix][saveVideoDataAsync] " + ex.message);
			...
			if (response.ok == false) {
				...
				console.error(new Date().toLocaleString() + " -- [Netflix][saveVideoDataAsync] " + error, data);
				result.error = error;
				if (data["error"] != null)
					result.error += "\n" + data["error"];
